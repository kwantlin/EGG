...
class _BatchIterator:
    ...
    def get_batch(self):
        ...
        inds = self.random_state.choice(len(self.loader.dataset.samples), opt.game_size*opt.batch_size, replace=False)
        # print(inds)
        ims = []
        for i in inds:
            ims.append(self.loader.dataset[i][0])
        ims = torch.stack(ims, dim=0)
        # print("ims shape", ims.shape)
        feats = self.model(ims)
        # print("feats shape", feats.shape)

        images_vectors_sender = torch.stack(torch.split(feats,int(feats.shape[0]/2)))
        ...
        # print(images_vectors_sender.shape, y.shape, images_vectors_receiver.shape)
...
class ImageNetFeat(data.Dataset):
    def __init__(self, root, train=True):
        ...
        # print(self.data[49][0].shape)
        # print(self.data.samples[0])
        # # FC features
